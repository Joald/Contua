...
kindCheckType (TArrow t1 t2) = do
  (_, s1) <- kindCheckType t1
...
kindCheckVariants (TypeVariant _ args:tvs) = do
  subst  <- kindCheckVariantArgs args
...
doKindCheckIAST (IAST types fns) = do
  env <- monomorphise' <$> kindCheckTypeDecls types
  x <- local (const env) $ mapM kindCheckType $ catMaybes $ map ifnType fns ++ map ifnContType fns
  void $ local (const env) $ foldAdjacentM unifyKinds compose nullSubst $ map fst x ++ [KStar]
  return env
...
unifyKinds (KArrow l1 r1) (KArrow l2 r2) = do
  s1 <- unifyKinds l1 l2
